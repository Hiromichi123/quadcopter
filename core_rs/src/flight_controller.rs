...
use rclrs::Clock;
use rclrs::time::Duration;
use crate::{quadcopter::Quadcopter, target::Target, velocity::Velocity};
...
impl FlightController {
    ...
    // 严格位置检查
    pub fn pos_check_strict(&self, target: &mut Target, distance_x: f64, distance_y: f64, distance_z: f64) -> bool {
        if target.reached { return true; }
    ...
    pub fn fly_by_vel_duration(&mut self, velocity: &mut Velocity, duration: f64) {
        ...
        while (Clock::now().unwrap() - start_time).as_secs_f64() < duration {
            // 修改z轴反馈
